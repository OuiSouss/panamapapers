...
for i in countries:
	...
	for j in countries:
		if (i==j):
			...
			l_relations = []
			result = session.run("MATCH (n)-[r]->(m) WHERE (n.countries = '"+i+"' AND m.countries = '"+j+"') RETURN type(r) AS types")
			for t in result:
				h = t["types"]
				c = h.split(";")
				for s in c:
